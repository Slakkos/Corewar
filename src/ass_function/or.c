...
int check_or_format(char **params) {
    int count = 0;
...
static int handle_registre_case(int *args_or, char **bytescode_arr,
    robot_t *robot, int *cursor_int, int size)
{
    ...
    args_or[size] = robot->instructions_int[*cursor_int];
    (*cursor_int)++;
    robot->pc += 1;
    robot->cursor += 1;
    return 0;
...
static int handle_direct_indirect_case(int *args_or, char **bytescode_arr,
    robot_t *robot, int *cursor_int, int size)
{
...
static int fill_args_or(int *args_or, char **bytescode_arr,
    robot_t *robot, int *cursor_int)
{
    for (int size = 0; size < 3; size++) {
        if (handle_registre_case(args_or, bytescode_arr,
            robot, cursor_int, size) == 84) {
            return 84;
        }
        if (handle_direct_indirect_case(args_or, bytescode_arr,
            robot, cursor_int, size) == 84) {
            return 84;
        }
    }
...
static void print_debug_info(char *bytescode, char **bytescode_arr, int *args_or)
{
    my_putstr("                         --------------------------------\n");
    my_putstr("                                       or\n");
    my_putstr("                         --------------------------------\n");
    ...
    my_putstr("                         --------------------------------\n");
    my_putstr("                                  arguments_or\n");
    my_putstr("                                     ");
    print_int_array(args_or, 3);
    my_putstr("                         --------------------------------\n");
}

static int choose_or_param(char **bytescode_arr, robot_t* robot, int *args_or)
{
    if (!my_strcmp(bytescode_arr[0], "direct") == 0 ||
        !my_strcmp(bytescode_arr[0], "indirect") == 0) {
        args_or[0] = args_or[0];
    } else {
        args_or[0] = robot->registres[args_or[0]];
    }
    if (!my_strcmp(bytescode_arr[1], "direct") == 0 ||
        !my_strcmp(bytescode_arr[1], "indirect") == 0) {
        args_or[1] = args_or[1];
    } else {
        args_or[1] = robot->registres[args_or[1]];
    }
    return 0;
}
...
int or_function(st_t *st, robot_t *robot)
{
    ...
    int *args_or = malloc(sizeof(int) * 3);

    bytescode_arr = rename_content_bytescode(bytescode_arr);
    if (check_or_format(bytescode_arr) != 0) {
        free(args_or);
        return 84;
    }
    if (fill_args_or(args_or, bytescode_arr, robot, &cursor_int) == 84) {
        free(args_or);
        return 84;
    }
    print_debug_info(bytescode, bytescode_arr, args_or);
    choose_or_param(bytescode_arr, robot, args_or);
    robot->registres[args_or[2]] = args_or[0] | args_or[1];
    robot->pc += 2;
    robot->cursor += 2;
    robot->wait += 6;
    free(args_or);
